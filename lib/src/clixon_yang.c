              ...
              yang_stmt    *ywhen)
{
     ...
     */
    if (ysorig) {
        yang_stmt *m0 = yang_mymodule_get(ys);
        yang_stmt *m1 = yang_mymodule_get(ysorig);
        if (m0 == NULL)
            m0 = ys_module(ys);
        if (m1 == NULL)
            m1 = ys_module(ysorig);
        /* Only mirror if origin has an explicit when (not per-uses added). */
        if (m0 && m1 && m0 == m1 &&
            yang_find(ysorig, Y_WHEN, NULL) != NULL) {
            ys2 = ysorig;
...
/*! Print one-level yang spec
 *
 * Special pretty-print if yn is yang-spec
 * @param[in]  f    File to print to.
 * @param[in]  yn   Yang node to print
 */
int
            ...
            yang_stmt *yn)
{
    ...
    if (yn == NULL){
        clixon_err(OE_YANG, EINVAL, "yn is NULL");
        return -1;
    }
